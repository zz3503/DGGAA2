...
@DataTransforms.register_module()
class PointCloudHeightsNormalize(object):
    def __init__(self, **kwargs):
        pass

    def __call__(self, data):
        data['heights'] = data['heights'] / 100.0
        return data


@DataTransforms.register_module()
class BoxDropGPU(object):
    def __init__(self, color_drop=0.2, **kwargs):
        self.color_drop = color_drop

    def __call__(self, data):
        if torch.rand(1) < self.color_drop:
            min_xyz = data['pos'].min(dim=0).values
            max_xyz = data['pos'].max(dim=0).values
            inner_min, inner_max = self.random_inner_box(min_xyz, max_xyz)
            inside_mask = (
                    (data['pos'] >= inner_min) &
                    (data['pos'] <= inner_max)
            ).all(dim=1)
            ...
            num_inside = inside_idx.numel()
            if outside_idx.numel() > num_inside:
                rand_choice = outside_idx[torch.randint(
                    0, outside_idx.numel(), (num_inside,), device=data['pos'].device
                )]
                data['pos'][inside_idx] = data['pos'][rand_choice]
                data['x'][inside_idx] = data['x'][rand_choice]
                data['y'][inside_idx] = data['y'][rand_choice]
                data['heights'][inside_idx] = data['heights'][rand_choice]
        return data

    def random_inner_box(self, min_xyz, max_xyz, ratio=0.5, alpha=0.5):
        size = max_xyz - min_xyz
        inner_size = size * ratio
        max_offset = size - inner_size
        beta = torch.distributions.Beta(alpha, alpha)
        u = beta.sample((3,)).to(min_xyz.device)
        rand_offset = u * max_offset
        inner_min = min_xyz + rand_offset
        inner_max = inner_min + inner_size
        return inner_min, inner_max


@DataTransforms.register_module()
class ZDropGPU(object):
    def __init__(self, color_drop=0.2, ratio=0.01, **kwargs):
        self.color_drop = color_drop
        self.ratio = ratio
    ...
    def __call__(self, data):
        if torch.rand(1) < self.color_drop:
            ...
            thresh = (c_max - c_min) * self.ratio

            if torch.rand(1) < 0.5:
                drop_idx = coord <= c_min + thresh  # Ç° 5%
                keep_idx = coord > c_min + thresh
            else:
                drop_idx = coord >= c_max - thresh  # ºó 5%
                keep_idx = coord < c_max - thresh

            if keep_idx.sum() > drop_idx.sum():
                rand_idx = torch.randint(
                    0, keep_idx.sum(),
                    (drop_idx.sum(),),
                    device=data['pos'].device
                )
                data['pos'][drop_idx] = data['pos'][keep_idx][rand_idx]
                data['x'][drop_idx] = data['x'][keep_idx][rand_idx]
                data['y'][drop_idx] = data['y'][keep_idx][rand_idx]
                data['heights'][drop_idx] = data['heights'][keep_idx][rand_idx]
        return data
