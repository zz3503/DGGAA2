...
from openpoints.cpp import pointnet2_cuda, dynamic_graph_topk
...
def darwdiff(support_xyz, group_indices, query_xyz, center=0, camera_position=None):
    # 假设你的数据已经加载
    # points: (2048, 3), indices: (2048, 32)
    points = support_xyz[0].cpu().numpy()
    indices = group_indices[0].cpu().numpy()

    # 初始化绘图器
    plotter = pv.Plotter()
    if camera_position is not None:
        plotter.camera_position = camera_position
    # 提前创建 PolyData 对象，我们稍后只更新它的 scalars
    cloud = pv.PolyData(points)

    # 定义更新函数
    def update_neighborhood(value):
        target_idx = 5  # int(value)
        neighbor_indices = indices[target_idx]

        # 构造标签数组
        # 0: 其他, 1: 邻域, 2: 查询中心点
        labels = np.zeros(points.shape[0])
        labels[neighbor_indices] = 1
        labels[neighbor_indices[center]] = 2

        # 更新数据并刷新渲染
        cloud["group"] = labels
        # 清除旧的 mesh 以防重叠（或者使用特定的 name 覆盖）
        plotter.add_mesh(
            cloud.threshold([0, 0.5]), color="lightgray", point_size=20.0,
            render_points_as_spheres=True, opacity=0.7, name="others"
        )
        plotter.add_mesh(
            cloud.threshold([0.5, 1.5]), color="royalblue", point_size=20.0,
            render_points_as_spheres=True, name="neighbors"
        )
        plotter.add_mesh(
            cloud.threshold([1.5, 2.5]), color="red", point_size=25.0,
            render_points_as_spheres=True, name="center"
        )

    # 添加滑动条控制 target_idx
    plotter.add_slider_widget(
        callback=update_neighborhood,
        rng=[0, query_xyz.shape[1]],
        value=0,
        title="Target Point Index",
        pointa=(0.1, 0.1), pointb=(0.4, 0.1),  # 设置滑块在屏幕上的位置
        style='modern'
    )

    # 首次手动调用一次，初始化显示
    update_neighborhood(0)

    plotter.show()
    return plotter.camera_position


class DynamicGraphGroup(nn.Module):
    def __init__(self,
                 radius,
                 nsample,
                 **kwargs):
    ...
    def forward(self, query_xyz, support_xyz, features, idx):
        """
        :param query_xyz: (B, M, 3)
        :param support_xyz: (B, N, 3)
        :param features: (B, C, N)
        :param idx: (B, M)
        :return:
            p: (B, 3, M, G)
        ...
        group_indices = dynamic_graph_topk(support_xyz, features, idx, self.nsample, self.radius).int()

        # camera_position = darwdiff(support_xyz, group_indices, query_xyz, -1)
        # group_indices1 = ball_query(self.radius, self.nsample, support_xyz, query_xyz)
        # darwdiff(support_xyz, group_indices, query_xyz, 0,camera_position)
        # _, group_indices2 = KNN(self.nsample, transpose_mode=True)(support_xyz, query_xyz)
        # darwdiff(support_xyz, group_indices, query_xyz, 0,camera_position)
        ...
        # import numpy as np
        #
        # data1 = torch.zeros(query_xyz.shape[1], support_xyz.shape[1])
        # for i in range(query_xyz.shape[1]):
        #     data1[i][group_indices[0][i]] = 1
        # data2 = torch.zeros(query_xyz.shape[1], support_xyz.shape[1])
        # for i in range(query_xyz.shape[1]):
        #     data2[i][group_indices1[0][i]] = 1
        # data3 = torch.zeros(query_xyz.shape[1], support_xyz.shape[1])
        # for i in range(query_xyz.shape[1]):
        #     data3[i][group_indices2[0][i]] = 1
        # data1 = data1.t()
        # data2 = data2.t()
        # data3 = data3.t()
        # fig, axes = plt.subplots(1, 3, figsize=(12, 5))
        # sns.heatmap(data1, ax=axes[0], cmap='RdBu_r', center=0, cbar=False)
        # sns.heatmap(data2, ax=axes[1], cmap='RdBu_r', center=0, cbar=False)
        # sns.heatmap(data3, ax=axes[2], cmap='RdBu_r', center=0)
        # axes[0].axis('off')
        # axes[1].axis('off')
        # axes[2].axis('off')
        # plt.tight_layout()
        ...
        grouped_xyz = grouped_xyz - query_xyz.transpose(1, 2).unsqueeze(-1)  # relative position
        grouped_xyz /= self.radius
        grouped_features = grouping_operation(features, group_indices) if features is not None else None
        return grouped_xyz, grouped_features
...
def create_grouper(group_args):
    ...
    if nsample is not None:
        ...
        elif method == 'dynamicgraph':
            grouper = DynamicGraphGroup(radius, nsample, **group_args_copy)
    else:
        grouper = GroupAll()
    return grouper
