...
class AggregationLayer(nn.Module):
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__()
        assert reduction in ['sum', 'max', 'mean', 'attn', 'max+attn']
    ...
    def forward(self, f):
        if self.reduction == 'sum':
            ...
            f, a = torch.split(f, f.shape[1] // 2, dim=1)

            # import matplotlib.pyplot as plt
            # import seaborn as sns
            # import numpy as np
            #
            # data1 = torch.zeros(f.shape[1], f.shape[3])
            # for i, j in enumerate(torch.argmax(f[0, :, 0, :], dim=1)):
            #     data1[i, j] = 1
            # data1 = data1.cpu().numpy()
            # data2 = torch.softmax(a / 0.1, dim=-1)[0, :, 0, :].cpu().numpy()
            # data3 = torch.softmax(a / 0.2, dim=-1)[0, :, 0, :].cpu().numpy()
            # data4 = torch.softmax(a / 0.5, dim=-1)[0, :, 0, :].cpu().numpy()
            # data5 = torch.ones(f.shape[1], f.shape[3]).cpu().numpy() / 32
            # fig, axes = plt.subplots(1, 5, figsize=(12, 5))
            # sns.heatmap(data1, ax=axes[0], cmap='RdBu_r', vmin=0, vmax=0.5, center=0, cbar=False)
            # sns.heatmap(data2, ax=axes[1], cmap='RdBu_r', vmin=0, vmax=0.5, center=0, cbar=False)
            # sns.heatmap(data3, ax=axes[2], cmap='RdBu_r', vmin=0, vmax=0.5, center=0, cbar=False)
            # sns.heatmap(data4, ax=axes[3], cmap='RdBu_r', vmin=0, vmax=0.5, center=0, cbar=False)
            # sns.heatmap(data5, ax=axes[4], cmap='RdBu_r', vmin=0, vmax=0.5, center=0)
            # axes[0].axis('off')
            # axes[1].axis('off')
            # axes[2].axis('off')
            # axes[3].axis('off')
            ...
            a = torch.dropout(a, 0.05, self.training)
            f = (f * a).sum(-1)
        elif self.reduction == 'max+attn':
            f, a = torch.split(f, f.shape[1] // 2, dim=1)
            a = torch.softmax(a / self.temperature, dim=-1)
            a = torch.dropout(a, 0.05, self.training)
            f = f.amax(-1) + (f * a).sum(-1)
        return f
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        channels = [in_channels] + [mid_channel] * \
                   (layers - 1) + [out_channels * 2 if not self.is_head and 'attn' in reduction else out_channels]
        channels[0] = in_channels if is_head else CHANNEL_MAP[feature_type](channels[0])

        if self.use_res:
            self.skipconv = create_convblock1d(in_channels, out_channels, norm_args=None,
                                               act_args=None) if in_channels != out_channels else nn.Identity()
        ...
        if not is_head:
            ...
            self.grouper_name = group_args.NAME
            self.grouper = create_grouper(group_args)
            self.pool = AggregationLayer(reduction, **kwargs)
    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            if self.grouper_name == 'dynamicgraph' and not self.all_aggr:
                dp, fj = self.grouper(new_p, p, f, idx)
            else:
...
@MODELS.register_module()
class DGGAAPartDecoder(nn.Module):
    def __init__(self,
                 encoder_channel_list: List[int],
                 decoder_layers: int = 2,
        ...
        if self.cls_map == 'curvenet':
            ...
            self.global_conv1 = nn.Sequential(
                create_convblock1d(fp_channels[-2] * 2, 64,
                                   norm_args=None,
                                   act_args=act_args))
            skip_channels[0] += 64 + 128 + num_classes  # shape categories labels
